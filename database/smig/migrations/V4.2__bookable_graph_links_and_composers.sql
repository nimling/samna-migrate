DROP FUNCTION IF EXISTS claimius.get_graph_links_claim(UUID, UUID, INTEGER, TEXT[], TEXT[], UUID, INTEGER);
...
BEGIN
    ...
    IF NOT claimius.check_user_active(p_app_id, p_user_id) THEN
        RETURN jsonb_build_object('link', '[]'::jsonb);
    END IF;
    ...
    RETURN jsonb_build_object('link', v_links);
END;
...
DROP FUNCTION IF EXISTS public.get_owner_graph(UUID, UUID, UUID, INTEGER, INTEGER, TEXT[], TEXT[], TEXT[]);
DROP FUNCTION IF EXISTS public.get_claim_graph(UUID, UUID, UUID, INTEGER, INTEGER, TEXT[], TEXT[], TEXT[]);
DROP FUNCTION IF EXISTS public.get_relation_graph(UUID, UUID, UUID, INTEGER, INTEGER, TEXT[], TEXT[], TEXT[]);
...
DECLARE
    v JSONB;
BEGIN
    v := claimius.get_graph_objects(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, NULL::UUID[], p_with_image, p_compact)
         || claimius.get_graph_links_owner(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, p_start_id, p_depth);
...
BEGIN
    v := claimius.get_graph_objects(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, NULL::UUID[], p_with_image, p_compact)
         || claimius.get_graph_links_claim(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, p_start_id, p_depth);

    IF 'owner' = ANY(p_with) THEN
        v := claimius.merge_graphs(v, claimius.get_graph_links_owner(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, NULL::UUID, p_depth));
    END IF;
...
BEGIN
    ...
    v := claimius.get_graph_objects(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, NULL::UUID[], p_with_image, p_compact)
         || jsonb_build_object('link', v_links);

    IF 'owner' = ANY(p_with) THEN
        v := claimius.merge_graphs(v, claimius.get_graph_links_owner(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, p_start_id, p_depth));
    END IF;
