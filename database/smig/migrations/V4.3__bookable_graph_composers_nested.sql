...
BEGIN
    v := jsonb_build_object(
        ...
        'link',    COALESCE(claimius.get_graph_links_owner(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, p_start_id, p_depth) -> 'link', '[]'::jsonb)
    );
...
BEGIN
    v := jsonb_build_object(
        'objects', claimius.get_graph_objects(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, NULL::UUID[], p_with_image, p_compact),
        'link',    COALESCE(claimius.get_graph_links_claim(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, p_start_id, p_depth) -> 'link', '[]'::jsonb)
    );

    IF 'owner' = ANY(p_with) THEN
        v := claimius.merge_graphs(v, jsonb_build_object(
            'objects', '{}'::jsonb,
            'link',    COALESCE(claimius.get_graph_links_owner(p_user_id, p_app_id, p_required_access, p_include_types, p_exclude_types, NULL::UUID, p_depth) -> 'link', '[]'::jsonb)
        ));
    END IF;
