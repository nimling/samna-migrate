DROP FUNCTION IF EXISTS claimius.get_graph_objects(UUID, UUID, INTEGER, TEXT[], TEXT[], UUID[]);
...
BEGIN
    ...
    rows AS (
        ...
        JOIN visible v ON v.object_id = u.id AND v.object_type = 'claimius.samna_user'
        ...
        JOIN visible v ON v.object_id = c.id AND v.object_type = 'claimius.claim'
